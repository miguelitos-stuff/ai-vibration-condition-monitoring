...
if not SENSOR:
    ...
    for i in range(len(noise_list[0])):
        new_data = fn.generate_data(test_dict["data"], test_dict["label"][:, 1], noise_list[0][i]).float().to(device)
        output = fn.run_save_graph(model, new_data, f"{name}_{noise_list[1][i]}_noise")
    ...
    for i in range(len(noise_list[0])):
        new_data = fn.generate_data(test_dict["data"], test_dict["label"][:, 1], noise_list[0][i]).float().to(device)
        output = fn.run_save_graph(model, new_data, f"{name}_test_sensor_{sen_name}_{noise_list[1][i]}_noise")
